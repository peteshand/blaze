package blaze.away3d.managers 
{
	 ...
	 * @author P.J.Shand
	 */
	public class VirtualCursor3DManager 
	{
		...
		private var _virtualPoint:VirtualPoint;
		...
		private var _virtualPoints:Vector.<VirtualPoint>;
		private var _virtualPointFromId:Dictionary;
		...
		{
			super();
			_virtualPoints = new Vector.<VirtualPoint>();
			_virtualPointFromId = new Dictionary();
		...
		public function updateCollider():void
		{
			...
			if (_forceTouchMove || _updateDirty) { // If forceTouchMove is off, and no 2D Touch events dirty the update, don't update either.
				for (var i:uint; i < _numTouchPoints; ++i) {
					_virtualPoint = _virtualPoints[ i ];
					_collidingObject = _touchPicker.getViewCollision(_virtualPoint.x, _virtualPoint.y, _view);
					_collidingObjectFromTouchId[ _virtualPoint.id ] = _collidingObject;
				}
			}
		}
		...
		public function fireTouchEvents():void
		{
			...
			for (i = 0; i < _numTouchPoints; ++i) {
				...
				_collidingObject = _collidingObjectFromTouchId[ _virtualPoint.id ];
				_previousCollidingObject = _previousCollidingObjectFromTouchId[ _virtualPoint.id ];
				if (_collidingObject != _previousCollidingObject) {
					if (_previousCollidingObject)
						queueDispatch(TouchEvent3D.TOUCH_OUT, _touchMoveEvent, _previousCollidingObject, _virtualPoint);
					if (_collidingObject)
						queueDispatch(TouchEvent3D.TOUCH_OVER, _touchMoveEvent, _collidingObject, _virtualPoint);
				}
				// Fire Touch move events here if forceTouchMove is on.
				if (_forceTouchMove && _collidingObject)
					queueDispatch(TouchEvent3D.TOUCH_MOVE, _touchMoveEvent, _collidingObject, _virtualPoint);
			}
			...
			for (i = 0; i < _numTouchPoints; ++i) {
				_virtualPoint = _virtualPoints[ i ];
				_previousCollidingObjectFromTouchId[ _virtualPoint.id ] = _collidingObjectFromTouchId[ _virtualPoint.id ];
			}
		}
		...
		private function queueDispatch(emitType:String, sourceEvent:TouchEvent, collider:PickingCollisionVO, touch:VirtualPoint):void
		{
		...
		{
			/*var touch:VirtualPoint = new VirtualPoint();
			touch.id = event.touchPointID;
			touch.x = event.stageX;
			touch.y = event.stageY;
			_numTouchPoints++;
			_virtualPoints.push(touch);
			_virtualPointFromId[ touch.id ] = touch;
		...
		{
			/*var touch:VirtualPoint = _virtualPointFromId[ event.touchPointID ];
		...
		private function onTouchEnd():void
		{
			...
			_virtualPointFromId[ touch.id ] = null;
			_numTouchPoints--;
			_virtualPoints.splice(_virtualPoints.indexOf(touch), 1);
