package blaze.behaviors 
{
	...
	public class AwayTouchBehavior extends BaseTouchBehavior 
	{
		...
		public function AwayTouchBehavior(Begin:Function, Move:Function, End:Function, Over:Function, Out:Function) 
		{
		...
		public function addListenerTo(touchObject:Entity, iPickingCollider:IPickingCollider=null, useStageForRelease:Boolean=false):void
		{
			...
			if (Move != null)						touchObject.addEventListener(TouchEvent3D.TOUCH_MOVE, OnTouchEventMove);
			if (End != null)						touchObject.addEventListener(TouchEvent3D.TOUCH_END, OnTouchEventEnd);
			...
			if (active){
				if (!pressCooldownActive) {
					if (useStageForRelease) {
						core.stage.addEventListener(MouseEvent.MOUSE_UP, OnStageMouseUp);
						core.stage.addEventListener(MouseEvent.MOUSE_MOVE, OnStageMouseMove);
					}
		...
		private function OnStageMouseUp(event:MouseEvent):void 
		{
			if (active){
				if (!releaseCooldownActive) {
					if (useStageForRelease) {
						core.stage.removeEventListener(MouseEvent.MOUSE_UP, OnStageMouseUp);
						core.stage.removeEventListener(MouseEvent.MOUSE_MOVE, OnStageMouseMove);
					}
