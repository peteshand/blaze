package blaze.behaviors 
{
	...
	import blaze.behaviors.LanguageChangeBehavior;
	 ...
	 * @author Pete Shand
	 */
	public class SuperSceneChangeBehavior extends BaseObject 
	{
		...
		public function SuperSceneChangeBehavior(Show:Function, Hide:Function) 
		{
			super();
		...
		{
			removeSceneListener();
			sceneChangeBehavior = new SceneChangeBehavior(sceneIndex, ShowScene, HideScene);
		}
		...
		private function addSceneIndicesListener():void 
		{
			...
			for (var i:int = 0; i < sceneIndices.length; ++i) {
				...
				sceneIndicesChangeBehaviors.push(new SceneIndicesChangeBehavior(sceneIndices[i], i, showSceneIndices, checkVis));
			}
		}
		...
		private function addSubSceneListener():void 
		{
			...
			subSceneChangeBehavior = new SubSceneChangeBehavior(subSceneIndex, ShowSubScene, HideSubScene);
		}
		...
		private function addSubSceneIndicesListener():void 
		{
			...
			for (var i:int = 0; i < subSceneIndices.length; ++i) {
				...
				subSceneIndicesChangeBehaviors.push(new SubSceneIndicesChangeBehavior(subSceneIndices[i], i, showSubSceneIndices, checkVis));
			}
		}
		...
		private function addLanguageListener():void 
		{
			...
			languageChangeBehavior = new LanguageChangeBehavior(languageIndex, ShowLanguage, HideLanguage);
		}
...
class SubSceneIndicesChangeBehavior
{
	...
	public function SubSceneIndicesChangeBehavior(subSceneIndex:int, index:int, showSubSceneIndices:Vector.<Boolean>, callback:Function)
	{
		this.index = index;
		this.showSubSceneIndices = showSubSceneIndices;
		this.callback = callback;
		subSceneChangeBehavior = new SubSceneChangeBehavior(subSceneIndex, Show, Hide);
	}
...
class SceneIndicesChangeBehavior
{
	...
	public function SceneIndicesChangeBehavior(sceneIndex:int, index:int, showSceneIndices:Vector.<Boolean>, callback:Function)
	{
		this.index = index;
		this.showSceneIndices = showSceneIndices;
		this.callback = callback;
		sceneChangeBehavior = new SceneChangeBehavior(sceneIndex, Show, Hide);
	}
